...
class SimpleSteem:  
    ...
    def __init__(self, **kwargs):
        ...
        self.sc = steemconnectutil.SteemConnect(self.client_id, 
    ...
    def steem_instance(self):
        ...
        for x in range(3):
            try:
                self.s = Steem(keys=self.keys, nodes=[self.util.goodnode(self.nodes)])
            except Exception as e:
                self.util.retry("COULD NOT GET STEEM INSTANCE", e, num_of_retries, 60)
            else:
                return self.s
        return False
    ...
    def claim_rewards(self):
        if self.steem_instance().claim_reward_balance(account=self.mainaccount):
            time.sleep(5)
            return True
        else:
            return False
    ...
    def current_vote_value(self, lastvotetime=None, steempower=0, voteweight=100, votepower=0):
        c = Converter()
    ...
    def transfer_funds(self, to, amount, denom, msg):
        try:
            self.steem_instance().commit.transfer(to, float(amount), denom, msg, self.mainaccount)
        except Exception as e:
            self.msg.error_message(e)
            return False
        else:
    ...
    def post(self, title, body, permlink, tags):
        for num_of_retries in range(4):
            try:
                self.steem_instance().post(title, body, self.mainaccount, permlink, None, None, None, None, tags, None, True)   
            except Exception as e:
                self.util.retry("COULD NOT POST '" + title + "'", e, num_of_retries, 10)
            else:
                time.sleep(60)
                ...
                if checkident == ident:
                    ...
                    self.util.retry("A POST JUST CREATED WAS NOT FOUND IN THE BLOCKCHAIN '" + title + "'", e, num_of_retries, 30)
    ...
    def reply(self, permlink, msgbody):
        for num_of_retries in range(4): 
            try:
                self.steem_instance().post("message", msgbody, self.mainaccount, None, permlink, None, None, "", None, None, False)
            except Exception as e:
                self.util.retry("COULD NOT REPLY TO " + permlink, e, num_of_retries, 30)
            else:
    ...
    def follow(self, author):
        try:
            self.steem_instance().commit.follow(author, ['blog'], self.mainaccount)
        except Exception as e:
            self.msg.error_message(e)
            return False
        else:
    ...
    def unfollow(self, author):
        try:
            self.steem_instance().commit.unfollow(author, ['blog'], self.mainaccount)
        except Exception as e:
            self.msg.error_message(e)
            return False
        else:
    ...
    def recent_post(self, author=None, daysback=0):
        ...
        for num_of_retries in range(4):
            try:
                ...
                self.util.retry("COULD NOT GET THE MOST RECENT POST FOR " + author, e, num_of_retries, 10)
            else:
                for p in blog:
                    age = self.util.days_back(p['comment']['created'])
                    if p['comment']['author'] == author and age == daysback:
                        return self.util.identifier(p['comment']['author'], p['comment']['permlink'])
    ...
    def vote(self, identifier, weight):
        for num_of_retries in range(4):
            try:
                self.steem_instance().vote(identifier, 100.0, self.mainaccount)
                ...
                if re.search(r'You have already voted in a similar way', str(e)):
                    ...
                    self.util.retry("COULD NOT VOTE ON " + identifier, e, num_of_retries, 10)
            else:
                return True
    ...
    def resteem(self, identifier):
        for num_of_retries in range(4):
            try:
                ...
                self.util.retry("COULD NOT RESTEEM " + identifier, e, num_of_retries, 10)
                return False
            else:
                return True
...
if __name__ == "__main__":
    ...
    print ("        Following " + str(num_of_followed) + " Steemians\n")
